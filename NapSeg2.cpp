...
int main( int argc, char** args )
{
    ...
    for ( int i = 1; i < n; ++i ) {
        ...
        vector<RMQ> segs(items[i].price, RMQ(len+1));

        for (t_v p = 0; p <= SUMP; ++p ) {
            t_v segs_no = p%items[i].price;
            
            segs[segs_no].update(0, len+1, make_pair(items[i].cal, 1));
            segs[segs_no].overwrite(cnt[segs_no], make_pair(dp[i-1][p].first, 0));
            cnt[segs_no] = (cnt[segs_no]+1)%(len+1);
    ...
    cout << "solving\t\t: " << "DP + SegmentTree(ver2)" << endl;
    cout << "SUMP\t\t: " << SUMP << endl;
