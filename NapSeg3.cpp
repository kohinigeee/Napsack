...
int main( int argc, char** args )
{
    ...
    for ( int i = 1; i < n; ++i ) {
        ...
        len = min(items[i].d, len);

        t_v cnt = 0;
        RMQ seg(len+1);

        for ( t_v q = 0; q < items[i].price; ++q ) {
          seg.update(0, len+1, make_pair(MINV, 0));
          cnt = 0;
          t_v p = q;
          while( p <= SUMP ) {
            seg.update(0, len+1, make_pair(items[i].cal, 1));
            seg.overwrite(cnt, make_pair(dp[i-1][p].first, 0));
            cnt = (cnt+1)%(len+1);
    ...
    cout << "solving\t\t: " << "DP + SegmentTree(ver3)" << endl;
    cout << "SUMP\t\t: " << SUMP << endl;
